  ...
  return (
    <section id="sponsors" className="py-24 bg-gradient-to-b from-gray-50 to-white dark:from-primary dark:to-primary/80 overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden py-10 my-4">
        ...
        <div className="absolute top-0 left-0 h-full w-24 md:w-40 bg-gradient-to-r from-gray-50 to-transparent dark:from-primary dark:to-transparent z-20 pointer-events-none"></div>
        <div className="absolute top-0 right-0 h-full w-24 md:w-40 bg-gradient-to-l from-gray-50 to-transparent dark:from-primary dark:to-transparent z-20 pointer-events-none"></div>

        <div className="flex relative z-10">
            {/* First set of logos */}
            <motion.div
              className="flex flex-shrink-0 flex-nowrap items-center px-4"
              animate={{ x: "-100%" }}
              transition={{
                ease: "linear",
                duration: 40,
            ...
            >
              {allSponsors.map((sponsor) => (
                <div
                  key={sponsor.id}
                  className="flex-shrink-0 flex items-center justify-center h-32 w-56 md:w-64 bg-white dark:bg-white/5 rounded-2xl shadow-sm border border-gray-100 dark:border-white/10 p-6 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 mx-4 group"
                >
                  <img
                    src={sponsor.logo}
                    alt={sponsor.name}
                    className="h-full w-full object-contain max-h-20 filter grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500"
                  />
                </div>
              ))}
            </motion.div>
